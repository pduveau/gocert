...
type bmpEncoder []rune

func (b bmpEncoder) Len() int {
	return len(utf16.Encode(b)) * 2
}

func (b bmpEncoder) Encode(dst []byte) {
	for i, u := range utf16.Encode(b) {
		dst[i*2] = byte(u >> 8)
		dst[i*2+1] = byte(u & 0x00FF)
	}
}
...
func makeBody(value reflect.Value, params fieldParameters) (e encoder, err error) {
	...
	case bigIntType:
		...
		return makeBigInt(v)
	case utf8StringType:
		v, _ := reflect.TypeAssert[UTF8String](value)
		return makeUTF8String(string(v))
	case ia5StringType:
		v, _ := reflect.TypeAssert[IA5String](value)
		return makeIA5String(string(v))
	case numericStringType:
		v, _ := reflect.TypeAssert[NUMERICString](value)
		return makeNumericString(string(v))
	case t61StringType:
		v, _ := reflect.TypeAssert[T61String](value)
		return makeT61String(v)
	case bmpStringType:
		v, _ := reflect.TypeAssert[BMPString](value)
		return makeBMPString(v)
	}
	...
	case reflect.String:
		...
		default:
			return makeUTF8String(v.String())
		}
	}
