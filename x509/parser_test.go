...
func TestParseASN1String(t *testing.T) {
	tests := []struct {
		...
		{
			name:     "T61String",
			tag:      pkixstring.T61String,
			value:    []byte{0xbf, 0x61, 0x3f},
			expected: asn1.T61String("¿a?"),
		},
		{
			name:     "PrintableString",
			tag:      pkixstring.PrintableString,
			value:    []byte{80, 81, 82},
			expected: string("PQR"),
		},
		{
		...
		{
			name:     "BMPString",
			tag:      pkixstring.Tag(asn1.TagBMPString),
			value:    []byte{80, 81},
			expected: asn1.BMPString("偑"),
		},
		{
		...
		{
			name:     "NumericString",
			tag:      pkixstring.Tag(asn1.TagNumericString),
			value:    []byte{49, 50},
