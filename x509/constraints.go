...
func checkChainConstraints(chain []*Certificate) (err error) {
	...
	for i, c := range chain {
		...
		var dns = []string{}
		var uris = []parsedURI{}
		var emails = []rfc2821Mailbox{}
		var ipAddresses = []net.IP{}
		if c.San != nil {
			uris, err = parseURIs(c.San.URIs)
			if err != nil {
				return
			}
			emails, err = parseMailboxes(c.San.EmailAddresses)
			if err != nil {
				return
			}
			dns = c.San.DNSNames
			ipAddresses = c.San.IPAddresses
		}

		for n := currentConstraints; n != nil; n = n.next {
			if err := n.check(dns, uris, emails, ipAddresses); err != nil {
				return err
			}
		}
	}
