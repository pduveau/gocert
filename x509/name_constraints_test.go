...
func makeConstraintsLeafCert(leaf leafSpec, key *ecdsa.PrivateKey, parent *Certificate, parentKey *ecdsa.PrivateKey) (*Certificate, error) {
	...
	dirnames := pkix.Name{}.AppendRDN(pkix.OidOrganizationalUnit, "leaf").AppendRDN(pkix.OidCommonName, leaf.cn)
	template := &Certificate{
		SerialNumber:          new(big.Int).SetBytes(serialBytes[:]),
		Subject:               dirnames,
		NotBefore:             time.Unix(1000, 0),
	...
	for _, name := range leaf.sans {
		...
		case strings.HasPrefix(name, "dns:"):
			san.DNSNames = append(san.DNSNames, name[4:])
			template.San = &san
		...
		case strings.HasPrefix(name, "ip:"):
			...
			san.IPAddresses = append(san.IPAddresses, ip)
			template.San = &san
		...
		case strings.HasPrefix(name, "invalidip:"):
			...
			san.IPAddresses = append(san.IPAddresses, net.IP(ipBytes))
			template.San = &san

		case strings.HasPrefix(name, "email:"):
			san.EmailAddresses = append(san.EmailAddresses, name[6:])
			template.San = &san
