...
func nameToKey(name *pkix.Name) string {
	return strings.Join(name.Country, ",") + "/" + strings.Join(name.Organization, ",") + "/" + strings.Join(name.OrganizationalUnit, ",") + "/" + strings.Join(name.CommonName, ",")
}
...
func generatePEMCertWithRepeatSAN(currentTime int64, count int, san string) string {
	...
	if ip := net.ParseIP(san); ip != nil {
		cert.San.IPAddresses = slices.Repeat([]net.IP{ip}, count)
	} else {
		cert.San.DNSNames = slices.Repeat([]string{san}, count)
	}
...
func generateCert(cn string, isCA bool, issuer *Certificate, issuerKey crypto.PrivateKey) (*Certificate, crypto.PrivateKey, error) {
	...
	template := &Certificate{
		...
		Subject:      pkix.Name{}.AppendRDN(pkix.OidCommonName, cn),
...
func genCertEdge(t *testing.T, subject string, key crypto.Signer, mutateTmpl func(*Certificate), certType int, issuer *Certificate, signer crypto.Signer) *Certificate {
	...
	tmpl := &Certificate{
		SerialNumber: serial,
		Subject:      pkix.Name{}.AppendRDN(pkix.OidCommonName, subject),
	...
	case leafCertificate:
		tmpl.San = &SubjectAlternativeName{DNSNames: []string{"localhost"}}
	}
	if mutateTmpl != nil {
		mutateTmpl(tmpl)
	}
				...
				Graph: []trustGraphEdge{
					...
					},
					{
						...
						MutateTemplate: func(t *Certificate) {
							...
							t.San = &SubjectAlternativeName{DNSNames: []string{"bad"}}
						},
					},
					{
						Issuer:  "inter a",
				...
				Graph: []trustGraphEdge{
					{
						...
						MutateTemplate: func(c *Certificate) {
							c.San = &SubjectAlternativeName{DNSNames: []string{"localhost"}}
						},
					},
				},
			},
				...
				Graph: []trustGraphEdge{
					{
						...
						Type:    intermediateCertificate,
						MutateTemplate: func(t *Certificate) {
							t.San = &SubjectAlternativeName{DNSNames: []string{"beep.com"}}
						},
					},
					{
						Issuer:  "inter",
						...
						Type:    leafCertificate,
						MutateTemplate: func(t *Certificate) {
							t.San = &SubjectAlternativeName{DNSNames: []string{"www.example.com"}}
						},
					},
				},
			},
	...
	for _, tc := range []struct {
		...
		t.Run(fmt.Sprintf("root EKUs %#v, verify EKUs %#v", tc.rootEKUs, tc.verifyEKUs), func(t *testing.T) {
			tmpl := &Certificate{
				...
				Subject:      pkix.Name{}.AppendRDN(pkix.OidCommonName, "root"),
				NotBefore:    time.Now().Add(-time.Hour),
				NotAfter:     time.Now().Add(time.Hour),
...
func TestCertificateChainSignedByECDSA(t *testing.T) {
	...
	root := &Certificate{
		SerialNumber:          big.NewInt(1),
		Subject:               pkix.Name{}.AppendRDN(pkix.OidCommonName, "X"),
		NotBefore:             time.Now().Add(-time.Hour),
	...
	leaf := &Certificate{
		...
		Subject:               pkix.Name{}.AppendRDN(pkix.OidCommonName, "leaf"),
